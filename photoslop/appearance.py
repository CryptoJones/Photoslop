...
EFFECT_DEFAULTS: dict[str, dict] = {
    "drop-shadow": {"offset_x": 6, "offset_y": 6, "blur": 8, "spread": 0, "color": [0, 0, 0, 153]},
    "inner-shadow": {"offset_x": 3, "offset_y": 3, "blur": 5, "spread": 0, "color": [0, 0, 0, 140]},
    "outer-glow": {"size": 10, "spread": 0, "color": [255, 220, 120, 200]},
    "inner-glow": {"size": 8, "choke": 0, "source": "edge", "color": [255, 255, 255, 180]},
...
def _legacy_effect(value) -> dict:
    ...
    effect["id"] = uuid.uuid5(uuid.NAMESPACE_URL, "photoslop-effect:" + repr(tuple(value))).hex
    return effect
...
def effect_margin(effects) -> int:
    ...
    for effect in normalize_effects(effects):
        ...
        if kind == "drop-shadow":
            margin = max(
                margin, round(p["blur"] + p["spread"] + max(abs(p["offset_x"]), abs(p["offset_y"])))
            )
        elif kind in {"outer-glow", "outline"}:
            margin = max(margin, round(p.get("size", p.get("width", 0)) + p.get("spread", 0)))
...
def _gradient_image(img: QImage, p: dict) -> QImage:
    ...
    t = np.clip(((xx - cx) * math.cos(angle) + (yy - cy) * math.sin(angle)) / span + 0.5, 0, 1)
    if p["reverse"]:
        t = 1 - t
...
def render(layer) -> RenderedAppearance:
    ...
    source = (
        layer.paint_image(layer.image.rect()) if layer.mask is not None else QImage(layer.image)
    )
    ...
    for effect in effects:
        ...
        if kind == "drop-shadow":
            ...
            off = source_offset + QPoint(round(p["offset_x"]) - pad, round(p["offset_y"]) - pad)
            ...
            planes.append(
                Plane(
                    _color_image(plane, p["color"]),
                    source_offset - QPoint(pad, pad),
                    True,
                    blend,
                    opacity,
                )
            ...
            if position == "inside":
                ...
                if position == "center":
                    ...
                    plane[
                        outside : outside + source.height(), outside : outside + source.width()
                    ] += inside
                image = _color_image(plane, p["color"])
            ...
            planes.append(
                Plane(_color_image(plane, p["color"]), source_offset, False, blend, opacity)
            )
            ...
            if p["source"] == "center":
                ...
                edge = np.clip(
                    alpha - _morph(alpha, max(1, size + round(p["choke"])), False), 0, 255
                )
            ...
            planes.append(
                Plane(_color_image(alpha, p["color"]), source_offset, False, blend, opacity)
            )
        elif kind == "gradient-overlay":
            planes.append(Plane(_gradient_image(source, p), source_offset, False, blend, opacity))
            ...
            planes.append(
                Plane(
                    _color_image(highlight, p["highlight_color"]),
                    source_offset,
                    False,
                    "screen",
                    opacity,
            ...
            planes.append(
                Plane(
                    _color_image(shadow, p["shadow_color"]),
                    source_offset,
                    False,
                    "multiply",
                    opacity,
    ...
    return RenderedAppearance(
        planes, fill_override, QPoint(source_offset) if fill_override is not None else None
    )
...
BUILTIN_PRESETS: dict[str, list[dict]] = {
    ...
    "Sticker": [
        new_effect("drop-shadow", offset_x=4, offset_y=5, blur=5),
        new_effect("outline", width=6, color=[255, 255, 255, 255]),
    ],
    "Neon": [
        new_effect("outer-glow", size=16, spread=2, color=[0, 220, 255, 230]),
        new_effect("inner-glow", size=5, color=[255, 255, 255, 210]),
    ],
    "Letterpress": [
        new_effect("inner-shadow", offset_x=2, offset_y=2, blur=3),
        new_effect("bevel-emboss", depth=60, size=2),
    ],
    "Chrome": [
        new_effect("gradient-overlay", color1=[245, 250, 255, 255], color2=[45, 65, 90, 255]),
        new_effect("bevel-emboss", depth=180, size=5),
    ],
    "Soft Focus": [
        new_effect("gaussian-blur", radius=2),
        new_effect("outer-glow", size=8, color=[255, 255, 255, 130]),
    ],
}
