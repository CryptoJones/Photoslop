        ...
        for key, value in element.attrib.items():
            ...
            if local == "href":
                if value.startswith(svg_prefix):
                    try:
                        nested = base64.b64decode(value[len(svg_prefix) :], validate=True)
                    except (binascii.Error, ValueError) as exc:
                        raise ValueError("SVG: invalid embedded SVG") from exc
                    nested_root = parse_xml_limited(nested, operation="embedded SVG")
                    _validate_resource_refs(nested_root)
                elif not (value.startswith("#") or value.startswith("data:image/png;base64,")):
...
def _commands(source: str) -> list[dict]:
    ...
    while index < len(tokens):
        ...
        if upper in {"M", "L"}:
            ...
            commands.append(
                {"op": "C", "c1": points[0], "c2": points[1], "p": points[2], "node": "smooth"}
            )
    return commands


def _style(element, gradients: dict) -> dict:
    values = dict(item.split(":", 1) for item in element.get("style", "").split(";") if ":" in item)
    ...
    return {
        ...
        "dash": [float(value) for value in re.findall(_NUMBER, values.get("stroke-dasharray", ""))],
...
def _gradient(element) -> dict:
    kind = _tag(element)
    paint = {
        "type": "linear-gradient" if kind == "linearGradient" else "radial-gradient",
        "stops": [],
    }
    if kind == "linearGradient":
        ...
        paint.update(
            center=[float(element.get("cx", 0.5)), float(element.get("cy", 0.5))],
            radius=float(element.get("r", 0.5)),
        )
    for stop in element:
        if _tag(stop) != "stop":
            continue
        style = dict(item.split(":", 1) for item in stop.get("style", "").split(";") if ":" in item)
        color = _color(
            stop.get("stop-color", style.get("stop-color", "black")),
...
def load_svg(path: str, *, allow_large: bool = False) -> Document:
    source = read_limited(path, DESKTOP_BUDGET.max_xml_bytes, operation="SVG")
    ...
    validate_dimensions(width, height, operation="SVG canvas", buffers=2, allow_large=allow_large)
    doc = Document(QSize(width, height), 72, path)
    gradients = {
        element.get("id"): _gradient(element)
        for element in root.iter()
        if _tag(element) in {"linearGradient", "radialGradient"} and element.get("id")
    }
    unsupported = sorted(
        {_tag(element) for element in root.iter() if _tag(element) not in _SUPPORTED}
    )
    for element in root.iter():
        tag = _tag(element)
        geometry = None
            ...
            if href.startswith(prefix):
                ...
                if not image.isNull():
                    validate_dimensions(
                        image.width(),
                        image.height(),
                        operation="SVG embedded image",
                        buffers=2,
                        allow_large=allow_large,
                    )
                    layer = Layer(
                        element.get("id", "SVG image"),
                        image,
                        QPoint(
        ...
        data = vector.migrate_vector(
            {
                ...
                "parent_id": None,
                "extensions": {},
                "text": ({"content": "".join(element.itertext())} if tag == "text" else None),
            }
        )
    ...
    metadata = next(
        (
            element
            for element in root.iter()
            if _tag(element) == "metadata" and element.get("id") == "photoslop"
        ),
        None,
    )
    if metadata is not None and metadata.text:
        ...
        doc.artboards = [
            (name, QRect(x, y, w, h)) for name, x, y, w, h in payload.get("artboards", [])
        ]
...
def _paint(paint: dict | None, ids: dict, definitions: list[str]) -> str:
    ...
    if key not in ids:
        ...
        attrs = (
            f'cx="{paint["center"][0]}" cy="{paint["center"][1]}" r="{paint["radius"]}"'
            if radial
            else f'x1="{paint["start"][0]}" y1="{paint["start"][1]}" '
...
def _filter_definition(filter_id: str, effects: list[dict], margin: int, bounds: QRect) -> str:
    ...
    for effect in normalize_effects(effects):
        ...
        if kind == "drop-shadow":
            ...
            if kind == "color-overlay":
                ...
                paint = (
                    f'<feImage href="data:image/svg+xml;base64,{encoded}" '
    ...
    size = max(1, margin)
    return (
        f'<filter id="{filter_id}" x="{bounds.x() - size}" '
    ...
    while block.isValid():
        ...
        while not iterator.atEnd():
            ...
            if fragment.isValid():
                ...
                position = (
                    f'x="{x}" dy="{0 if line == 0 else font.pointSizeF() * 1.2}"' if first else ""
                )
                pieces.append(
                    f"<tspan {position} {' '.join(attrs)}>{escape(fragment.text())}</tspan>"
                )
                first = False
            iterator += 1
        line += 1
...
def _write_svg(doc: Document, path: str) -> None:
    ...
    for index, layer in enumerate(doc.layers):
        ...
        if effects:
            ...
            definitions.append(
                _filter_definition(filter_id, effects, effect_margin(effects), layer.bounds())
            )
        ...
        style = (
            f'fill="{_paint(appearance.get("fill"), gradient_ids, definitions)}" '
        ...
        common = (
            f'id="{escape(data["id"])}" data-name="{escape(data["name"])}" '
        ...
        objects.append(obj)
    boards = [
        [name, rect.x(), rect.y(), rect.width(), rect.height()] for name, rect in doc.artboards
    ]
    metadata = escape(
        json.dumps(
            {"artboards": boards, "appearances": appearances, "text_layers": text_layers},
            separators=(",", ":"),
        )
    )
    source = (
        f'<svg xmlns="http://www.w3.org/2000/svg" width="{doc.size.width()}" '
