...
def validate_dimensions(
    ...
    *,
    ...
    if width > budget.max_dimension or height > budget.max_dimension:
        raise ResourceLimitError(f"{operation}: maximum dimension is {budget.max_dimension:,} px")
    pixels = width * height
...
def read_limited(path: str, maximum: int, *, operation: str) -> bytes:
    ...
    if size > maximum:
        raise ResourceLimitError(f"{operation}: file is {size:,} bytes; maximum is {maximum:,}")
...
def parse_xml_limited(
    ...
    *,
    ...
    if len(source) > budget.max_xml_bytes:
        raise ResourceLimitError(f"{operation}: XML exceeds {budget.max_xml_bytes:,} bytes")
    try:
    ...
    while stack:
        ...
        if count > budget.max_xml_nodes:
            raise ResourceLimitError(f"{operation}: XML exceeds {budget.max_xml_nodes:,} nodes")
        if depth > budget.max_xml_depth:
            raise ResourceLimitError(f"{operation}: XML nesting exceeds {budget.max_xml_depth}")
        stack.extend((child, depth + 1) for child in node)
    return root


def validate_archive_members(
    infos, *, operation: str = "archive", budget: ResourceBudget = DESKTOP_BUDGET
) -> dict:
    """Validate ZIP metadata before reading any member bytes."""
    infos = list(infos)
    if len(infos) > budget.max_archive_entries:
        raise ResourceLimitError(f"{operation}: archive has too many entries ({len(infos)})")
    total = 0
    by_name = {}
    for info in infos:
        ...
        if ratio > budget.max_compression_ratio:
            raise ResourceLimitError(f"{operation}: suspicious compression ratio for {name!r}")
...
def read_archive_member(
    zf, info, *, operation: str, budget: ResourceBudget = DESKTOP_BUDGET
) -> bytes:
